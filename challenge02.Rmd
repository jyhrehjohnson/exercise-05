...
head(z)
#can we use z by itself? or do we need to select for height, weight, age, zombies killed and YOE??
```
...
for (i in seq_along(y)) {
  s_mean <- mean(y[[i]])
  s_sd <- sd(y[[i]])
  s_se <- s_sd / sqrt(50) #this throws off knitting
...
survivors199 <- tibble(
  ...
  a_mean = do(a) * mean(~age, data = sample_n(z, size = n1, replace = FALSE)),
...
#survivors199 <- tibble(
  #z_seq = c(1:199),
  #height_mean = do(199) * mean(z$height, na.rm = TRUE),
  #weight_mean = do(199) * mean(z$weight, na.rm = TRUE),
  #age_mean = do(199) * mean(z$age, na.rm = TRUE),
  #zombieK_mean = do(199) * mean(z$zombies_killed, na.rm = TRUE),
  #ed_mean = do(199) * mean(z$years_of_education, na.rm = TRUE)
#)
...
h_meanDist <- (mean(~mean, data = survivors199$height_mean) + survivors50[[1,2]])/2 #do we need to /2?
h_meanDist

w_meanDist <- (mean(~mean, data = survivors199$weight_mean) + survivors50[[2,2]])/2
w_meanDist

a_meanDist <- (mean(~mean, data = survivors199$a_mean) + survivors50[[3,2]])/2
a_meanDist
...
a_meanSD <- sd(~mean, data = survivors199$a_mean)
...
histogram(~mean, data = survivors199$a_mean, main = "Sampling Distribution (Age Means)")
